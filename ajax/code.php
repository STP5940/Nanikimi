...
if(empty($_GET["code"])){
    die(error('กรุณากรอก Code'));
}else{
$SQL = $odb -> prepare("SELECT * FROM `code` WHERE `code` = :code and `claimby` = 0");
$SQL -> execute(array(':code' => $_GET["code"]));
$data = $SQL->fetch();
if($data){
    $plan = $data["plan"];
        switch ($plan) {
        case '50': $expire = strtotime("+1 day"); $membership = 1;  break;
        case '90': $expire = strtotime("+5 day"); $membership = 2;  break;
        case '150': $expire = strtotime("+10 day"); $membership = 3;  break;
        case '300': $expire = strtotime("+20 day"); $membership = 4;  break;
        case '500': $expire = strtotime("+31 day"); $membership = 5;  break;
    ...
    $SQL2 = $odb -> prepare("DELETE FROM `code` WHERE `code` = :code");
    $SQL2 -> execute(array(':code' => $_GET['code']));
    echo success('ใช้ Code สำเร็จ');
}else{
    die(error('Code ถูกใช้งานแล้ว หรือ ไม่มี Code ในระบบ'));
}
